...
/// @title ChessFish PaymentSplitter Contract
/// @author ChessFish
...
contract PaymentSplitter {
    ...
    address public immutable CFSH_token;
    ...
    function totalShares() public view returns (uint256) {
        return IERC20(CFSH_token).totalSupply();
    }
    ...
    function shares(address account) public view returns (uint256) {
        return IERC20(CFSH_token).balanceOf(account);
    }
    ...
    function releaseNative(address payable account) public {
        require(IERC20(CFSH_token).balanceOf(account) > 0, "PaymentSplitter: account has no shares");
    ...
    function _pendingPayment(
        ...
        return
            (totalReceived * IERC20(CFSH_token).balanceOf(account)) /
            IERC20Metadata(CFSH_token).totalSupply() -
            alreadyReleased;
    }
}
